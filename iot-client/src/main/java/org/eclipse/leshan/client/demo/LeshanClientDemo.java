...
public class LeshanClientDemo {
    ...
    public static LeshanClient createClient(LeshanClientDemoCLI cli, LwM2mModelRepository repository) throws Exception {
        ...
        if (cli.main.bootstrap) {
		    ...
		    // 2IMN15: for multi-server support, provide multiple
		    //         security objects with server addresses.
                    initializer.setInstancesForObject(SECURITY, noSec(cli.main.url, 123));
                }
		// 2IMN15: for multi-server support, provide multiple
		//         server objects with matching IDs.
                initializer.setInstancesForObject(SERVER, new Server(123, cli.main.lifetimeInSec));
            }
        }
	...
	if (cli.solarpanel.enabled) {
	    // Create the object.
	    SolarPanel solarPanel = new SolarPanel();
	    // Adjust resource of the object based on parameters
	    // Register the object to the system.
	    initializer.setInstancesForObject(SolarPanel.OBJECT_ID, solarPanel);
	}
	if (cli.budgetallocation.enabled) {
	    // Create the object.
	    BudgetAllocation budgetAllocation = new BudgetAllocation();
	    // Adjust resource of the object based on parameters
	    // Register the object to the system.
	    initializer.setInstancesForObject(BudgetAllocation.OBJECT_ID, budgetAllocation);
	}
	if (cli.battery.enabled) {
	    // Create the object.
	    Battery battery = new Battery();
	    // Adjust resource of the object based on parameters
	    // Register the object to the system.
	    initializer.setInstancesForObject(Battery.OBJECT_ID, battery);
	}
	//
