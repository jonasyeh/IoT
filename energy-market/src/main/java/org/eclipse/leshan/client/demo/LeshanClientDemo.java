...
public class LeshanClientDemo {
    ...
    public static LeshanClient createClient(LeshanClientDemoCLI cli, LwM2mModelRepository repository) throws Exception {
        ...
        if (cli.main.bootstrap) {
		    ...
		    // 2IMN15: add multiple servers based on cli options
		    //         (room control, energy control, flow control)
                    initializer.setInstancesForObject(SECURITY,
						      noSec(cli.main.url, 123),
						      noSec(cli.energycontrol.url, 124),
						      noSec(cli.flowcontrol.url, 125));
                }
		// 2IMN15: add multiple servers based on cli options  
                initializer.setInstancesForObject(SERVER,
						  new Server(123, cli.main.lifetimeInSec),
						  new Server(124, cli.main.lifetimeInSec),
						  new Server(125, cli.main.lifetimeInSec));
            }
        }
	...
	//         for energy market:
	//         Bid, Offer, Wallet, Marketplace State, Match
	// Since all objects are required for the scenarios, create
	// one for each of these objects.
	// Note that RoomControl, EnergyControl and FlowControl will
	// create additional objects.
	//
	MarketplaceState mstate = new MarketplaceState();
	initializer.setInstancesForObject(MarketplaceState.OBJECT_ID,
					  mstate);
	Bid bid = new Bid();
	initializer.setInstancesForObject(Bid.OBJECT_ID,
					  bid);
	Offer offer = new Offer();
	initializer.setInstancesForObject(Offer.OBJECT_ID,
					  offer);
	Wallet wallet = new Wallet();
	initializer.setInstancesForObject(Wallet.OBJECT_ID,
					  wallet);
	Match match = new Match();
	initializer.setInstancesForObject(Match.OBJECT_ID,
					  match);
	//
	//  2IMN15:  TODO
	//
	//  Extend the classes of those objects such that they
	//  can perform updates on the MarketPlace.
	//  That is:
	//  * when Bid objects are create or modified, update the
	//    marketplace.
	//  * similar for Offer and Wallet objects.
